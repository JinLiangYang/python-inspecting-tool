...
#执行show命令
def exec_cmd(cmd_list,log_folder,lic,port):
...
ssh.connect(hostname=host_ip, port=host_port, username=user, password=password,timeout=30)
...
channel.send('terminal width 512\n')
time.sleep(1)
#循环执行cmd.txt的命令
temp=''
out=''
...
print('***********************************\n')
#预读取内容
folder=create_logdir(host_ip) #创建日志文件夹
lic_name=get_license() #获取license信息
sc=get_interface()
time.sleep(1)
exec_cmd('cmd.txt',folder,lic_name,sc)#执行命令
print('日志收集完成！') 
